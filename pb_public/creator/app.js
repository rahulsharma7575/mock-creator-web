...
createApp({
  ...
  methods:{
    ...
    audioOrdered(){const want=['tts_model','tts_male_voice','tts_female_voice','tts_fallback_model','tts_fallback_male_voice','tts_fallback_female_voice','audio_gap_ms','sample_rate','tts_speed'];const by={};this.meta.forEach(m=>{if(m.group==='Audio')by[m.field]=m;});return want.map(f=>by[f]).filter(Boolean);},
    audioCol(field){if(field==='audio_gap_ms'||field==='sample_rate'||field==='tts_speed')return'a-span2';const prim=['tts_model','tts_male_voice','tts_female_voice'];return prim.includes(field)?'a-col1':'a-col2';},

...
createApp({
  data(){return{
    ...
    audioPool:{busy:false,id:'',active:false,count:0,file:'',updated:'',note:'',fileName:'',verify:null,error:'',drag:false,preview:[]},
    blankPool:{busy:false,id:'',active:false,count:0,file:'',updated:'',note:'',fileName:'',verify:null,error:'',drag:false,preview:[]},
  }},
  computed:{
  ...
  methods:{
    ...
    loadAll(){this.loadClients();this.loadJobs();this.loadConfigs();this.loadMeta();this.loadModels();this.loadTtsModels();this.loadAudioPool();this.loadFalStatus();this.loadOrBalance();this.loadGeminiStatus();this.loadMagnificStatus();this.loadUpstageStatus();this.loadHealth();this.ensureOrModels();this.$nextTick(()=>this.viewEnter());},
    ...
    audioVoices(prim){const defs=[
      {key:'tts_male_voice',speed:'tts_male_speed',label:'Male voice',prim:true,tip:'Male speaker (V1). Fish audio default: 802e3bc2b27e49c2995d23ef70e6ac89. Empty = auto-pick per TTS model.'},
      {key:'tts_female_voice',speed:'tts_female_speed',label:'Female voice',prim:true,tip:'Female speaker (V2). Fish audio default: 9a9cf47702da476aa4629e2506d4a857. Empty = auto-pick per TTS model.'},
    ...
    audioSettings(){return['audio_gap_ms','sample_rate','tts_natural_pacing','tts_polish'].map(f=>this.mf(f)).filter(m=>m.field);},
    ...
    toggleApiMenu(){this.apiMenu=!this.apiMenu;if(this.apiMenu)this.refreshAllStatus();},
    async loadHealth(){try{const r=await api('/api/creator/health');this.health=r;}catch(e){this.health=null;}},
    ...
    async loadAudioPool(){try{const r=await api('/api/creator/audio-pool-status');Object.assign(this.audioPool,{id:r&&r.id||'',active:!!(r&&r.active),count:(r&&r.count)||0,file:r&&r.file||'',updated:r&&r.updated||'',note:r&&r.note||''});Object.assign(this.blankPool,this.audioPool);}catch(e){this.audioPool.error=e.message;this.blankPool.error=e.message;}},
    async loadBlankPool(){return this.loadAudioPool();},
    verifyAudioJson(text){let arr=null;try{arr=JSON.parse(text);}catch(e){return{ok:false,error:'Not valid JSON: '+e.message};}if(!Array.isArray(arr))return{ok:false,error:'Root must be a JSON array of questions'};const errs=[];for(let i=0;i<arr.length;i++){const it=arr[i];const w=it&&typeof it==='object'?it.woman:null;if(!it||typeof it!=='object'||typeof it.man!=='string'||!it.man.trim()){errs.push('#'+(i+1)+': missing "man" question');}else if(!w||typeof w!=='object'){errs.push('#'+(i+1)+': missing "woman" options object');}else{for(const k of ['1','2','3','4']){const v=w[k];if(typeof v!=='string'||!v.trim())errs.push('#'+(i+1)+': woman.'+k+' empty');}}if(!(it.answer===1||it.answer===2||it.answer===3||it.answer===4))errs.push('#'+(i+1)+': answer must be 1-4');if(errs.length>8)break;}if(errs.length)return{ok:false,count:arr.length,totalBad:Math.min(arr.length,errs.length),errors:errs};return{ok:true,count:arr.length};},
    verifyBlankJson(text){return this.verifyAudioJson(text);},
    async handleAudioPoolFile(f){if(!f)return;this.audioPool.fileName=f.name;this.audioPool.error='';this.audioPool._file=f;this.audioPool.verify={checking:true};this.audioPool.preview=[];this.blankPool.fileName=f.name;this.blankPool._file=f;this.blankPool.verify={checking:true};try{const buf=await f.arrayBuffer();const bytes=new Uint8Array(buf);let text;if(bytes.length>=2&&bytes[0]===0x1f&&bytes[1]===0x8b){const ds=new DecompressionStream('gzip');const stream=new Blob([buf]).stream().pipeThrough(ds);text=await new Response(stream).text();}else{text=new TextDecoder('utf-8').decode(buf);}const v=this.verifyAudioJson(text);this.audioPool.verify=v;this.blankPool.verify=v;if(v.ok){try{const arr=JSON.parse(text);const prev=Array.isArray(arr)?arr.slice(0,10).map((it,i)=>({idx:i+1,man:String(it.man||'').slice(0,60),w1:String((it.woman||{})['1']||'').slice(0,24),w2:String((it.woman||{})['2']||'').slice(0,24),w3:String((it.woman||{})['3']||'').slice(0,24),w4:String((it.woman||{})['4']||'').slice(0,24),ans:it.answer})):[];this.audioPool.preview=prev;this.blankPool.preview=prev;}catch(e){}}}catch(e){const er={ok:false,error:'Could not read the file ('+e.message+')'};this.audioPool.verify=er;this.blankPool.verify=er;}},
    async onAudioPoolPick(ev){const f=ev.target&&ev.target.files&&ev.target.files[0];if(ev.target)ev.target.value='';await this.handleAudioPoolFile(f);},
    async onAudioPoolDrop(ev){this.audioPool.drag=false;const f=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];await this.handleAudioPoolFile(f);},
    async onBlankPoolPick(ev){return this.onAudioPoolPick(ev);},
    async uploadAudioPool(){const bp=this.audioPool;if(!bp._file)return this.toast('Pick a audio_questions.json first','warn');if(!(bp.verify&&bp.verify.ok))return this.toast('File did not pass verification - fix it first','warn');if(bp.busy)return;bp.busy=true;bp.error='';const n=(bp.verify&&bp.verify.count)||0;try{const t=localStorage.getItem(LS_TK);const fd=new FormData();fd.append('file',bp._file,bp._file.name||'audio_questions.json');let url='';let col='audio_pool';try{const chk=await api('/api/creator/audio-pool-status');if(chk&&chk.id)col='audio_pool';}catch(e){}url=bp.id?(`/api/collections/${col}/records/`+encodeURIComponent(bp.id)):(`/api/collections/${col}/records`);const r=await fetch(url,{method:bp.id?'PATCH':'POST',headers:{'Authorization':t?'Bearer '+t:''},body:fd});let b=null;try{b=await r.json();}catch(e){}if(!r.ok)throw new Error((b&&(b.message||b.error))||('HTTP '+r.status));bp._file=null;bp.verify=null;bp.fileName='';Object.assign(this.blankPool,bp);this.toast('Audio pool uploaded ('+n+' questions) - worker verifies on next run','ok');this.loadAudioPool();}catch(e){bp.error=e.message;this.toast('audio pool upload: '+e.message,'err');}bp.busy=false;},
    async uploadBlankPool(){return this.uploadAudioPool();},
    async resetAudioPool(){const bp=this.audioPool;if(bp.busy)return;if(!bp.id)return;bp.busy=true;try{const t=localStorage.getItem(LS_TK);const r=await fetch('/api/collections/audio_pool/records/'+encodeURIComponent(bp.id),{method:'DELETE',headers:{'Authorization':t?'Bearer '+t:''}});if(!r.ok){const r2=await fetch('/api/collections/blank_pool/records/'+encodeURIComponent(bp.id),{method:'DELETE',headers:{'Authorization':t?'Bearer '+t:''}});if(!r2.ok)throw new Error('HTTP '+r2.status);}this.toast('Audio pool reset - system bundled pool will be used','ok');this.loadAudioPool();}catch(e){bp.error=e.message;this.toast('reset: '+e.message,'err');}bp.busy=false;},
    async resetBlankPool(){return this.resetAudioPool();},
    ...
    retryJob(){const j=this.drawerJob;const k=j.kind||'full';if(Number(j.gen_type||1)>=2&&j.pdf){this.confirmMsg='Re-queue this PDF job for "'+j.client_name+'"? The stored PDF is re-attached and it runs as '+this.genTypeLabel(j).label+'.';this.confirmFn=async()=>{try{await this.requeueWithPdf(j,k);this.toast('PDF job re-queued','ok');this.drawerJob=null;this.loadJobs();}catch(e){this.toast('requeue: '+e.message,'err');}};}else{this.confirmMsg='Re-queue '+k+' job for "'+j.client_name+'" ('+j.count+'q)?';this.confirmFn=async()=>{try{await this.startJob({client:j.client,count:j.count,difficulty:j.difficulty||'',kind:k});this.toast('Re-queued','ok');this.drawerJob=null;this.loadJobs();}catch(e){this.toast('requeue: '+e.message,'err');}};}},
    async requeueWithPdf(j,kind){const t=localStorage.getItem(LS_TK);const headers={'Authorization':t?'Bearer '+t:''};const fr=await fetch('/api/files/mock_jobs/'+j.id+'/'+encodeURIComponent(j.pdf),{headers});if(!fr.ok)throw new Error('PDF fetch failed: HTTP '+fr.status);const blob=await fr.blob();const fd=new FormData();fd.append('pdf',blob,j.pdf);const q=new URLSearchParams({client:j.client,count:j.count||40,kind:kind||'full',gen_type:String(j.gen_type||3)});const r=await fetch('/api/creator/start?'+q.toString(),{method:'POST',headers:{'Authorization':t?'Bearer '+t:''},body:fd});let b=null;try{b=await r.json();}catch(e){}if(!r.ok)throw new Error((b&&(b.error||b.message))||('HTTP '+r.status));return b;},
    ...
    async copyJobLog(){const l=this.drawerJob&&(this.drawerJob.log||'');if(!l)return this.toast('No log yet','warn');try{await navigator.clipboard.writeText(l);this.toast('Log copied','ok');}catch(e){this.toast('copy failed','err');}},
    downloadJobLog(){const l=this.drawerJob&&(this.drawerJob.log||'');if(!l)return this.toast('No log yet','warn');const blob=new Blob([l],{type:'text/plain'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='job_'+String(this.drawerJob.id).slice(0,8)+'.log';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),5000);},
    openPdfTest(){if(this.pdfTest.busy)return;this.pdfTest={open:true,drag:false,busy:false,job:null,result:null,error:'',fileName:''};},
    closePdfTest(){if(this.pdfTest.busy)return;this.pdfTest.open=false;},
    runAgain(){this.pdfTest={open:true,drag:false,busy:false,job:null,result:null,error:'',fileName:''};},
    onPdfPick(e){const inp=e&&e.target;const f=inp&&inp.files&&inp.files[0];if(inp)inp.value='';if(f)this.runPdfTest(f);},
    pdfDrop(ev){this.pdfTest.drag=false;const f=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];if(f)this.runPdfTest(f);},
    async runPdfTest(file){if(!file)return this.toast('Pick a PDF first','warn');if(!this.gClient)return this.toast('Choose a client first','warn');if(this.pdfTest.busy)return;this.pdfTest={open:true,drag:false,busy:true,job:null,result:null,error:'',fileName:file.name};try{const t=localStorage.getItem(LS_TK);const fd=new FormData();fd.append('pdf',file);const q=new URLSearchParams({client:this.gClient,kind:'pdf_test',count:1,gen_type:'3'});const parser=this.cfgData&&this.cfgData.pdf_parser;if(parser&&parser!=='auto')q.set('parser',parser);const r=await fetch('/api/creator/start?'+q.toString(),{method:'POST',headers:{'Authorization':t?'Bearer '+t:''},body:fd});let b=null;try{b=await r.json();}catch(e){}if(!r.ok)throw new Error((b&&(b.error||b.message))||('HTTP '+r.status));this.pdfTest.job=b.job_id;this.toast('Parser test queued - parsing '+file.name,'info');this.pollPdfTest(b.job_id);}catch(e){this.pdfTest.busy=false;this.pdfTest.error=e.message;this.toast('pdf test: '+e.message,'err');}},
    async pollPdfTest(jid){for(let i=0;i<120;i++){await new Promise(r=>setTimeout(r,4000));let j=null;try{const r=await api('/api/creator/jobs/'+jid);j=r;}catch(e){continue;}if(j&&(j.status==='done'||j.status==='failed')){this.pdfTest.busy=false;if(j.status==='done'){this.pdfTest.result=j.report||{};this.toast('Parser test complete','ok');}else{this.pdfTest.error=j.error||'parser test failed';this.toast('Parser test failed','err');}return;}}this.pdfTest.busy=false;this.toast('Parser test still running - check the Jobs page','info');},
    ...
    delClient(c){this.confirmMsg='Permanently delete client "'+c.name+'"? Its config, jobs, dry runs and full runs are deleted too (cannot undo).';this.confirmFn=async()=>{try{const r=await api('/api/creator/delete-client?client='+encodeURIComponent(c.id),{method:'POST'});const d=r.deleted||{};this.toast('Client deleted — '+(d.jobs||0)+' jobs, '+(d.configs||0)+' configs removed','ok');this.loadClients();this.loadJobs();this.loadConfigs();}catch(e){this.toast('delete: '+e.message,'err');}};},

...
createApp({
  data(){return{
    ...
    pdfTest:{busy:false,job:null,result:null,error:''},
  ...
  methods:{
    ...
    audioOrdered(){const want=['tts_model','tts_male_voice','tts_female_voice','tts_fallback_model','tts_fallback_male_voice','tts_fallback_female_voice','audio_gap_ms','sample_rate','tts_speed','tts_natural_pacing','tts_polish','tts_atempo_models','listening_blank_count'];const by={};this.meta.forEach(m=>{if(m.group==='Audio')by[m.field]=m;});return want.map(f=>by[f]).filter(Boolean);},
    audioCol(field){const seq=['tts_model','tts_male_voice','tts_female_voice','tts_fallback_model','tts_fallback_male_voice','tts_fallback_female_voice','audio_gap_ms','sample_rate','tts_speed','tts_natural_pacing','tts_polish','tts_atempo_models','listening_blank_count'];const i=seq.indexOf(field);if(i<0)return'';return'a-c'+(i%3+1);},
    voicePh(field){const c=this.cfgData||{};const prim=field==='tts_male_voice'||field==='tts_female_voice';const model=String(prim?(c.tts_model||''):(c.tts_fallback_model||''));const male=field.includes('male_voice');if(model.includes('fish'))return male?'802e3bc2b27e49c2995d23ef70e6ac89':'933563129e564b19a115bedd57b7406a';return'Empty = auto-pick';},

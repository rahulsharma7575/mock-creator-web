...
createApp({
  data(){return{
    ...
    audioPool:{busy:false,id:'',active:false,count:0,file:'',updated:'',note:'',fileName:'',verify:null,error:''},
    blankPool:{busy:false,id:'',active:false,count:0,file:'',updated:'',note:'',fileName:'',verify:null,error:''},
  }},
  computed:{
  ...
  methods:{
    ...
    async onAudioPoolPick(ev){const f=ev.target&&ev.target.files&&ev.target.files[0];if(ev.target)ev.target.value='';if(!f)return;this.audioPool.fileName=f.name;this.audioPool.error='';this.audioPool._file=f;this.audioPool.verify={checking:true};this.blankPool.fileName=f.name;this.blankPool._file=f;this.blankPool.verify={checking:true};try{const buf=await f.arrayBuffer();const bytes=new Uint8Array(buf);let text;if(bytes.length>=2&&bytes[0]===0x1f&&bytes[1]===0x8b){const ds=new DecompressionStream('gzip');const stream=new Blob([buf]).stream().pipeThrough(ds);text=await new Response(stream).text();}else{text=new TextDecoder('utf-8').decode(buf);}const v=this.verifyAudioJson(text);this.audioPool.verify=v;this.blankPool.verify=v;}catch(e){const er={ok:false,error:'Could not read the file ('+e.message+')'};this.audioPool.verify=er;this.blankPool.verify=er;}},

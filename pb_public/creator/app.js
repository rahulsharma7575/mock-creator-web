...
createApp({
  ...
  methods:{
    ...
    onBlankPoolPick(ev){const f=ev.target&&ev.target.files&&ev.target.files[0];if(ev.target)ev.target.value='';if(!f)return;this.blankPool.fileName=f.name;this.blankPool.error='';this.blankPool._file=f;this.blankPool.verify={checking:true};f.text().then(t=>{this.blankPool.verify=this.verifyBlankJson(t);}).catch(()=>{this.blankPool.verify={ok:false,error:'Could not read the file'};});},

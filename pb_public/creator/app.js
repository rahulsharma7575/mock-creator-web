...
createApp({
  ...
  methods:{
    ...
    audioOrdered(){const want=['tts_model','tts_male_voice','tts_female_voice','tts_fallback_model','tts_fallback_male_voice','tts_fallback_female_voice','audio_gap_ms','sample_rate','tts_speed','tts_natural_pacing','tts_polish','tts_atempo_models'];const by={};this.meta.forEach(m=>{if(m.group==='Audio')by[m.field]=m;});return want.map(f=>by[f]).filter(Boolean);},
    audioCol(field){if(['audio_gap_ms','sample_rate','tts_speed','tts_natural_pacing','tts_polish','tts_atempo_models'].includes(field))return'a-span2';const prim=['tts_model','tts_male_voice','tts_female_voice'];return prim.includes(field)?'a-col1':'a-col2';},

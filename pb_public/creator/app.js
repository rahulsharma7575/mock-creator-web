...
createApp({
  ...
  methods:{
    ...
    loadAll(){this.loadClients();this.loadJobs();this.loadConfigs();this.loadMeta();this.loadModels();this.loadTtsModels();this.loadBlankPool();this.loadFalStatus();this.loadOrBalance();this.loadGeminiStatus();this.loadMagnificStatus();this.loadUpstageStatus();this.loadHealth();this.ensureOrModels();this.$nextTick(()=>this.viewEnter());},
    ...
    async loadBlankPool(){try{const r=await api('/api/creator/blank-pool-status');Object.assign(this.blankPool,{id:r&&r.id||'',active:!!(r&&r.active),count:(r&&r.count)||0,file:r&&r.file||'',updated:r&&r.updated||'',note:r&&r.note||''});}catch(e){this.blankPool.error=e.message;}},
    verifyBlankJson(text){let arr=null;try{arr=JSON.parse(text);}catch(e){return{ok:false,error:'Not valid JSON: '+e.message};}if(!Array.isArray(arr))return{ok:false,error:'Root must be a JSON array of questions'};const errs=[];for(let i=0;i<arr.length;i++){const it=arr[i];const w=it&&typeof it==='object'?it.woman:null;if(!it||typeof it!=='object'||typeof it.man!=='string'||!it.man.trim()){errs.push('#'+(i+1)+': missing "man" question');}else if(!w||typeof w!=='object'){errs.push('#'+(i+1)+': missing "woman" options object');}else{for(const k of ['1','2','3','4']){const v=w[k];if(typeof v!=='string'||!v.trim())errs.push('#'+(i+1)+': woman.'+k+' empty');}}if(!(it.answer===1||it.answer===2||it.answer===3||it.answer===4))errs.push('#'+(i+1)+': answer must be 1-4');if(errs.length>8)break;}if(errs.length)return{ok:false,count:arr.length,totalBad:Math.min(arr.length,errs.length),errors:errs};return{ok:true,count:arr.length};},
    async onBlankPoolPick(ev){const f=ev.target&&ev.target.files&&ev.target.files[0];if(ev.target)ev.target.value='';if(!f)return;this.blankPool.fileName=f.name;this.blankPool.error='';this.blankPool._file=f;this.blankPool.verify={checking:true};try{const buf=await f.arrayBuffer();const bytes=new Uint8Array(buf);let text;if(bytes.length>=2&&bytes[0]===0x1f&&bytes[1]===0x8b){const ds=new DecompressionStream('gzip');const stream=new Blob([buf]).stream().pipeThrough(ds);text=await new Response(stream).text();}else{text=new TextDecoder('utf-8').decode(buf);}this.blankPool.verify=this.verifyBlankJson(text);}catch(e){this.blankPool.verify={ok:false,error:'Could not read the file ('+e.message+')'};}},
    async uploadBlankPool(){const bp=this.blankPool;if(!bp._file)return this.toast('Pick a blank_questions.json first','warn');if(!(bp.verify&&bp.verify.ok))return this.toast('File did not pass verification - fix it first','warn');if(bp.busy)return;bp.busy=true;bp.error='';const n=(bp.verify&&bp.verify.count)||0;try{const t=localStorage.getItem(LS_TK);const fd=new FormData();fd.append('file',bp._file,bp._file.name||'blank_questions.json');const url=bp.id?('/api/collections/blank_pool/records/'+encodeURIComponent(bp.id)):'/api/collections/blank_pool/records';const r=await fetch(url,{method:bp.id?'PATCH':'POST',headers:{'Authorization':t?'Bearer '+t:''},body:fd});let b=null;try{b=await r.json();}catch(e){}if(!r.ok)throw new Error((b&&(b.message||b.error))||('HTTP '+r.status));bp._file=null;bp.verify=null;bp.fileName='';this.toast('Blank pool uploaded ('+n+' questions) - worker verifies on next run','ok');this.loadBlankPool();}catch(e){bp.error=e.message;this.toast('blank pool upload: '+e.message,'err');}bp.busy=false;},
    async resetBlankPool(){const bp=this.blankPool;if(bp.busy)return;if(!bp.id)return;bp.busy=true;try{const t=localStorage.getItem(LS_TK);const r=await fetch('/api/collections/blank_pool/records/'+encodeURIComponent(bp.id),{method:'DELETE',headers:{'Authorization':t?'Bearer '+t:''}});if(!r.ok)throw new Error('HTTP '+r.status);this.toast('Blank pool reset - system bundled pool will be used','ok');this.loadBlankPool();}catch(e){bp.error=e.message;this.toast('reset: '+e.message,'err');}bp.busy=false;},
